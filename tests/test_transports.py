...
async def test_qq_voice_attachment_processing(monkeypatch):
    ...
    monkeypatch.setattr("jarvis.media.transcode_wav_to_mp3", lambda d: b"mock-wav-mp3-bytes")
    ...
    # 1. Test AMR fallback (no voice_wav_url)
    mock_message = MagicMock()
    ...
    download_mock = AsyncMock(return_value=b"mock-amr-bytes")
    monkeypatch.setattr(bot, "_download", download_mock)
    ...
    # Verify we decoded mock-mp3-bytes
    import base64
    assert base64.b64decode(att.url.split(",", 1)[1]) == b"mock-mp3-bytes"
    assert received_msg.metadata.get("asr_text") == "hello from QQ ASR"
    download_mock.assert_called_with("http://qq.com/voice.amr")
    
    # 2. Test voice_wav_url preferred
    received_msg = None
    mock_message = MagicMock()
    mock_message.content = ""
    mock_message.author.user_openid = "user123"
    mock_message.id = "msg124"
    ...
    mock_att._raw_data = {
        "content_type": "voice",
        "url": "http://qq.com/voice.amr",
        "voice_wav_url": "http://qq.com/voice.wav",
        "asr_refer_text": "hello from QQ ASR"
    }
    mock_message.attachments = [mock_att]
    mock_message._api = mock_api
    
    download_mock = AsyncMock(return_value=b"mock-wav-bytes")
    monkeypatch.setattr(bot, "_download", download_mock)
    ...
    att = received_msg.attachments[0]
    assert att.mime_type == "voice"
    assert base64.b64decode(att.url.split(",", 1)[1]) == b"mock-wav-mp3-bytes"
    download_mock.assert_called_with("http://qq.com/voice.wav")
